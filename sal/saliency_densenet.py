...
class RTSaliencyModelDensenet(nn.Module):

    def __init__(self, encoder, encoder_chans, upsampler_scales,
                 upsampler_base, fix_encoder=True,
                 use_simple_activation=False, allow_selector=False, num_classes=1000):
        super(RTSaliencyModelDensenet, self).__init__()

        self.encoder = encoder
        self.encoder_scales = len(encoder_chans)
        self.upsampler_scales = upsampler_scales
        self.upsampler_base = upsampler_base
        self.fix_encoder = fix_encoder
        self.use_simple_activation = use_simple_activation
        self.encoder_chans = encoder_chans

        down = self.encoder_scales
        modulator_size = []
        for up in reversed(range(self.upsampler_scales)):
            upsampler_chans = upsampler_base * 2 ** (up + 1)
            encoder_chans = self.encoder_chans[down - 1]
            inc = upsampler_chans if down != self.encoder_scales else encoder_chans
            modulator_size.append(inc)
            self.add_module("up%d" % up,
                            UNetUpsampler(
                                in_channels=inc,
                                passthrough_channels=self.encoder_chans[down - 2],
    ...
    def forward(self, _images, _selectors=None, pt_store=None, model_confidence=0.):
        ...
        if self.allow_selector:
            ...
            act = torch.sum(main_flow * em.view(-1, self.encoder_chans[-1], 1, 1), 1, keepdim=True)
...
def get_pretrained_saliency_fn(ckpt_dir, cuda=True, return_classification_logits=False):
    ...
    '''
    saliency = RTSaliencyModelDensenet(densenet169encoder(pretrained=True), [64, 256, 512, 1280, 1664], 3, 64, fix_encoder=False,
